    ...
    foreach (int|string|decimal)[] item in rssFeedsInfo {
        ...
        if isInvalidEndpoint == true {
            log:printError(setAlertMessage(feedName + 
                " :- Invalid url or an error occures in the http module. Please check the entered url in the Spreadsheet."));
            currentRowNumber = currentRowNumber + 1;
            continue;
        }

        if response.statusCode != 200 {
            log:printError(setAlertMessage(feedName + " :- An error occurs when fetching the details from the endpoint." + 
